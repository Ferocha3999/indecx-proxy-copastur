...
const INDECX_URLS = ['https://indecx.com/v3/integrations/get-answers/all','https://indecx.com/v2/answers-info/all'];
const HEADERS = {'company-integration-key':COMPANY_KEY,'company-key':COMPANY_KEY,'Content-Type':'application/json'};
    ...
    try {
      ...
      if (totalPags > 1) {
        ...
        for (let p = 2; p <= Math.min(totalPags, 50); p++) {
          promises.push(fetch(baseUrl + '?limit=1000&page=' + p, { headers: HEADERS }).then(r => r.json()).then(j => j.answers || j.invites || j.data || j.dados || []).catch(() => []));
        }
        const pages = await Promise.all(promises);
        pages.forEach(p => { dados = dados.concat(p); });
      }
      return res.json({ dados, total: dados.length });
    } catch (err) { console.log('Erro:', err.message); continue; }
  }
